...
if 'ward_bound' not in st.session_state:
    st.session_state.ward_bound = None
...
if st.session_state.ward_data is not None:
    ...
    # Highlight function factory
    def make_highlight_function(column, colormap):
    ...
    for i, (column, name) in enumerate(layer_name_mapping.items()):
        ...
        layer = folium.GeoJson(
            ...
            style_function=make_style_function(column, colormap),
            highlight_function=make_highlight_function(column, colormap),
            show=(i == 0),
        )
        layer.add_to(m)
    ...
    st.error("Failed to load ward data. Please check the data files.")
